...
*`store` is where we keep important information. it stores the state of our application.
*`actions` are functions that dispatch command objects to our reducer. command objects should have a shape `{type: "COMMAND_STRING", payload: dataToPassToReducer}`
*`reducer` is a function that takes the information dispatched from our actions to update our state.
- [x] What is the difference between Application state and Component state? When would be a good time to use one over the other?
*`Application` state is state that spans multiple views, something like saved preferences or wether someone is logged in
*`Component state` is state local to a component, for instance if you have a form might be a good time for this. because the state of the whole application doesnt really need to be updated just because someone types in a controlled input field on a form
- [x] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?
*`redux-thunk` is cool because it allows us to create ASYNC actions that can change state multiple times depending on the state of a promise.
